...
newInstance.get(API.CONFIGS_MICSYSTEMS_ROUTERS).then(res => {
  if(res.status == 200){
    // 获取成功，存储配置文件结果
    MicRouters = Common.confirm(res, 'data', {})
    console.log('MicRouters',MicRouters,'appRoutes',appRoutes)
    // 初始化处理子系统路由的component
    for(let key in MicRouters){
      for(let i=0;i<MicRouters[key].length;i++){

        let firstRouter = Object.assign({}, MicRouters[key][i])
        if(!!firstRouter.meta && !!firstRouter.meta.openMode) {
          switch (firstRouter.meta.openMode) {
          case 'outsite':
            firstRouter['component'] = TipsOutsite
            appRoutes[0].children.push(firstRouter)
            break
          case 'spa':
