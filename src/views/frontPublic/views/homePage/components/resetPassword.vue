    ...
    <div class="findPassword">
      <div v-if="appearIndex == 0" class="linksTips">
        <div @click="appearIndex = 2">
          ...
          >
            <a-col span="6"
              ><img src="@/assets/images/head1.png" alt="" class="icon"
            /></a-col>
            <a-col span="16">
        ...
        <div @click="appearIndex = 1" class="unBindLinks">
          ...
          >
            <a-col span="6"
              ><img src="@/assets/images/head2.png" alt="" class="icon"
            /></a-col>
            <a-col span="16">
      ...
      >
          ...
          <a-row type="flex">
            ...
            <a-col :span="8" :offset="1">
              <a-button
                ...
                @click="sendCode"
                size="small"
                style="height:42px;min-width: 86px;"
              >
                {{ btnTxt }}
              </a-button>
            </a-col>
        ...
        <testStrong
          :pwd="
            !formRegister.getFieldValue('pwd')
              ? ''
              : formRegister.getFieldValue('pwd')
          "
...
export default {
  ...
  methods: {
    ...
    validateCode(rule, value, callback) {
      if (
        ...
        if (!/^\d{6}$/.test(value)) {
          ...
          this.$ajax
            .get({
              url: this.$api.POST_CHECK_PHONE_CODE.replace('{type}', 'back')
                .replace('{phone}', this.formRegister.getFieldValue('username'))
                .replace('{code}', value)
            })
            .then(res => {
              if (res.code != '200') {
    ...
    sendCode() {
      this.$ajax
        .get({
          url: this.$api.GET_PHONE_CODE.replace(
            '{phone}',
            this.formRegister.getFieldValue('username')
          ).replace('{type}', 'back')
        })
        .then(res => {
