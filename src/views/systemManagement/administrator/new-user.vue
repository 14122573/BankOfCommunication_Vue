    ...
    <p class="gayLine noline"></p>
		...
		<a-table size='small' class="portalTable" :columns="columns" :dataSource="dataTable" rowKey='id' :pagination='false'>
			...
			<span slot="action" slot-scope="text, record">
				...
				<a-divider type="vertical" v-if="permission('P03301')"  />
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { title: '角色名称', dataIndex: 'roleNames', key: 'roleNames', scopedSlots: { customRender: 'roleNames' } },
        { title: '所属组织机构', dataIndex: 'group', key: 'group', scopedSlots: { customRender: 'group' } },
        { title: '所属行政区域', dataIndex: 'area', key: 'area', scopedSlots: { customRender: 'area' } },
  ...
  methods: {
    ...
    handleOk() {
      ...
      switch (key) {
      case '1': //启用操作
        apiUrl = this.$api.CHECK_USER_STATUS.replace('{type}', 'new').replace('{id}', this.opeation.item.id).replace('{status}', '1')
        break
      case '2': //禁用操作
        apiUrl = this.$api.CHECK_USER_STATUS.replace('{type}', 'new').replace('{id}', this.opeation.item.id).replace('{status}', '9')
        break
      case '3': //注销操作
        apiUrl = this.$api.CHECK_USER_STATUS.replace('{type}', 'new').replace('{id}', this.opeation.item.id).replace('{status}', '8')
        break
      }
      this.$ajax.put({
        url: apiUrl
