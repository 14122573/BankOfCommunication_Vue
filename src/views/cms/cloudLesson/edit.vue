              ...
              <a-row :gutter='16'>
                <a-col span="16">
                  <a-form-item label="附件" :label-col="{span:4}" :wrapper-col="{span:20}" v-if="ready">
                    ...
                    <a-alert style="margin-top:16px" message="最多上传9个附件，每个附件不得大于10M，支持的格式为：word、excel、ceb、cebx、pdf" type="info" showIcon />
                  </a-form-item>
                </a-col>
              </a-row>
            </div>
  ...
  data() {
    return {
      ...
      rules     : {
        ...
        introduction: [
          { required: true, whitespace: true, message: '请输入云课堂简介!' },
        ...
        if (!err) {
          ...
          this.$ajax.post({
            ...
            if (res.code === '200') {
              ...
              case 'saveNcreate':
                description = '暂存并新建成功'
                this.$router.push({ name: '/cms/cloudlesson/create' })
                break
              case 'publish':
                description = '发布成功'
                this.$router.go(-1)
