<template>
  <div class="routerWapper">
    <div
      ...
      v-if="$route.name == '/cms/knowledgepromotion'"
    >
      <a-form class="protalForm" :form="knowledgePromotionSearchForm">
        <a-row
          class="formItemLine"
          type="flex"
        ...
        >
          ...
          <a-col span="6" class="algin-right">
            ...
            <a-button type="primary" @click="getKnowledgePromotionList">搜索</a-button>
            <a-button
              type="primary"
...
export default {
  ...
  beforeCreate() {
    if (this.$route.name == '/cms/knowledgepromotion') {
      this.knowledgePromotionSearchForm = this.$form.createForm(this)
    }
  },
  mounted() {
    if (this.$route.name == '/cms/knowledgepromotion') {
      ...
      this.getKnowledgePromotionList()
    }
  },
  watch   : {},
  computed: {
  ...
  methods: {
    ...
    reset() {
      ...
      this.knowledgePromotionSearchForm.setFieldsValue({
        title   : '',
        section : '',
        postDate: [],
        keywords: '',
     ...
     * 调用结构，查询表单要求的知识文库资料
     */
    getKnowledgePromotionList() {
      let searchParms
      searchParms = Object.assign(
        {},
        this.searchForm,
        {
          title_l: !this.knowledgePromotionSearchForm.getFieldValue('title')
            ? ''
            : this.knowledgePromotionSearchForm.getFieldValue('title'),
          section_l: !this.knowledgePromotionSearchForm.getFieldValue('section')
            ? ''
            : this.knowledgePromotionSearchForm.getFieldValue('section'),
          postDate_l: !this.knowledgePromotionSearchForm.getFieldValue('postDate')
            ? ''
            : this.knowledgePromotionSearchForm.getFieldValue('postDate'),
          keywords_l: !this.knowledgePromotionSearchForm.getFieldValue('keywords')
            ? ''
            : this.knowledgePromotionSearchForm.getFieldValue('keywords'),
          mainBody_l: !this.knowledgePromotionSearchForm.getFieldValue('mainBody')
            ? ''
            : this.knowledgePromotionSearchForm.getFieldValue('mainBody'),
          postMan_l: !this.knowledgePromotionSearchForm.getFieldValue('postMan')
            ? ''
            : this.knowledgePromotionSearchForm.getFieldValue('postMan'),
          allIn_l: !this.knowledgePromotionSearchForm.getFieldValue('allIn')
            ? ''
            : this.knowledgePromotionSearchForm.getFieldValue('allIn'),
